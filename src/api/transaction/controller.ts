...
export const getUserTransactions = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Initial query for transactions where the user is involved as the sender
    let [userTransactions]: [RowDataPacket[], any] = await pool.query(
      `SELECT t.transactionType, t.transactionId, t.createdAt
       FROM transaction t
       WHERE t.userId = ?
       ORDER BY t.createdAt DESC`,
    ...
    // If no transactions, search where user might be a receiver
    if (userTransactions.length === 0) {
      [userTransactions] = await pool.query<RowDataPacket[]>(
        `
          SELECT 'gift_transaction' AS transactionType, id AS transactionId, createdAt
          FROM gift_transaction
          WHERE receivedId = ?
          
          UNION ALL
          
          SELECT 'session_transaction' AS transactionType, id AS transactionId, createdAt
          FROM session_transaction
          WHERE stream_sessionId = ?

          UNION ALL
          
          SELECT 'withdraw_transaction' AS transactionType, id AS transactionId, createdAt
          FROM withdraw_transaction
          WHERE userId = ?
          
          UNION ALL
          
          SELECT 'topup_transaction' AS transactionType, id AS transactionId, createdAt
    ...
    // If still no transactions found, return the response
    if (userTransactions.length === 0) {
      return res.status(404).json({ success: true, message: "No transactions found" });
    }

    // Loop through each transaction to get detailed data based on type
    const transactions = await Promise.all(
      userTransactions.map(async (transaction: any) => {
        let transactionDetails = null;
        ...
        if (transaction.transactionType === 'gift_transaction') {
          ...
          if (giftTransactions.length > 0) {
            const giftTransaction = giftTransactions[0];
            const adjustedAmount = giftTransaction.receivedId === userId
              ? Math.round(giftTransaction.amount * 0.5 * 100) / 100 // Apply 50% reduction for receiver
              : Math.round(giftTransaction.amount * 100) / 100;
          ...
          if (sessionTransactions.length > 0) {
            const sessionTransaction = sessionTransactions[0];
            const adjustedAmount = sessionTransaction.stream_sessionId === userId
              ? Math.round(sessionTransaction.amount * 0.5 * 100) / 100 // Apply 50% reduction if user is stream session host
              : Math.round(sessionTransaction.amount * 100) / 100;
            ...
            transactionDetails = {
              ...
              createdAt: sessionTransaction.createdAt,
              amount: adjustedAmount,
              transactionType: sessionTransaction.userId === userId ? 'out' : 'in',
              senderName: dataUser[0].username || '',
              receiverName: sessionTransaction.receiverName || '',
            };
          }
          ...
          const [topupTransaction]: [RowDataPacket[], any] = await pool.query(
            `SELECT tt.amount, tt.userId, tt.createdAt, user.username AS senderName
             FROM topup_transaction tt
             LEFT JOIN user ON tt.userId = user.id
             WHERE tt.id = ?`,
            [transaction.transactionId]
          );

          if (topupTransaction.length > 0) {
            const transactionData = topupTransaction[0];
            transactionDetails = {
              giftName: "TOPUP",
              description: `Top-up of Rp ${Math.round(transactionData.amount * 100) / 100}`,
              createdAt: transactionData.createdAt,
    ...
    // Return success response with formatted transaction list
    res.json({ success: true, transactions: filteredTransactions });
  } catch (error: any) {
    console.error("Error fetching user transactions:", error);
    res.status(500).json({
...
//       return res
//         .status(404)
//         .json({ success: false, message: "No transactions found gift" });
//     }
